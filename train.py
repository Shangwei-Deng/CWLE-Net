...
from model.UT5 import MN as CWLENet #UT4 UT5都是目前的网络
...
parser.add_argument("--model_names", default=['CWLENet'], type=list, help="'PIP', 'SCTransNet', 'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'RISTDnet'")
parser.add_argument("--dataset_names", default=['NUAA-SIRST'], type=list) #改数据集
...
parser.add_argument("--begin_test", default=20, type=int)#100
...
parser.add_argument("--dataset_dir", default='/home/xxx/Dataset')
parser.add_argument("--batchSize", type=int, default=16, help="Training batch size")
parser.add_argument("--patchSize", type=int, default=256, help="Training patch size")


parser.add_argument("--save", default='/home/xxx/log', type=str, help="Save path of checkpoints")



parser.add_argument("--log_dir", type=str, default="/home/xxx/CWLENet", help='path of log files')
        ...
        if (idx_epoch + 1) >= opt.begin_test and (idx_epoch + 1) % opt.every_test == 0:
            ...
            with torch.no_grad():
                ...
                for idx_iter, (img, gt_mask, size, _) in enumerate(test_loader):
                    ...
                    loss = net.loss(pred, gt_mask.to(device))
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'CWLENet':
            if mode == 'train':
                self.model = CWLENet(1, 1, mode='train', deepsuper=True)
                ...
                self.model = CWLENet(1, 1, mode='test', deepsuper=True)
                if torch.cuda.device_count() > 1:
                    print("Let's use ", torch.cuda.device_count(), " GPUs!")
                    self.model = nn.DataParallel(self.model, device_ids=[0, 1])
